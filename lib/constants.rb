module TwelveNotes
  class Constants
  	...
  	def get_12_notes(type = :sharps)
  		...
  		val_minus_1 = nil
  		val_plus_1 = nil
  		random_values = Array.new(12)

  		note_values = (1..12).to_a

  		(1..12).each do |n|

  			puts note_values.inspect
  			...
  			if selected_value
  				val_minus_1 = (selected_value == 1 ? 12 : selected_value - 1)
  				val_plus_1 = (selected_value == 12 ? 1 : selected_value + 1)

  				temp_notes.delete(val_minus_1)
  				temp_notes.delete(val_plus_1)
  			end

  			# select a random note from temp_notes
  			selected_value = temp_notes.sample

  			puts "Selected #{selected_value}"

  			# and save the random note in random_values
  			random_values[n] = selected_value
